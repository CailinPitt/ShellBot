...
* Bot is pretty basic right now, and can only execute one word commands (such as `date`, `ls`, `pwd`). Need to update so it can execute more complicated commands (like `git pull origin master`, `cd ../this/is/a/really/long/and/nested/directory/wow`, `ping www.google.com`).
